...
import {
  ...
  saveCustomSpell, deleteCustomSpell,
...
export const useStore = create((set, get) => ({
  ...
  saveCustomSpell: async (spell) => {
    ...
    if (!userId) return;
    const creatorName = get().profile?.player_name || '';
    const payload = {
      ...spell,
      id: spell.id || crypto.randomUUID(),
      creator_name: spell.creator_name || creatorName,
    };
  ...
  // ── Custom Weapons (global) ─────────────────────────────────────────────────
  customWeapons: [],
  customWeaponsLoading: false,

  loadCustomWeapons: async () => {
    set({ customWeaponsLoading: true });
    const { data } = await getAllCustomWeapons();
    set({ customWeapons: data || [], customWeaponsLoading: false });
  },

  saveCustomWeapon: async (weapon) => {
    const userId = get().user?.id;
    if (!userId) return;
    const creatorName = get().profile?.player_name || '';
    const payload = {
      ...
      creator_name: weapon.creator_name || creatorName,
    };
    const { data, error } = await saveCustomWeapon(userId, payload);
    if (!error && data) {
      set(state => {
        const idx = state.customWeapons.findIndex(w => w.id === data.id);
        const weapons = [...state.customWeapons];
        if (idx >= 0) weapons[idx] = data; else weapons.push(data);
        return { customWeapons: weapons };
      });
    }
    return { data, error };
  },

  removeCustomWeapon: async (id) => {
    const { error } = await deleteCustomWeapon(id);
    if (!error) {
      set(state => ({ customWeapons: state.customWeapons.filter(w => w.id !== id) }));
    }
    return { error };
  },
}));
