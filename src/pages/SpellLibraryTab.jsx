...
// ── Spell detail modal ────────────────────────────────────────────────────────
function SpellDetailModal({ spell, isCustom, onClose, onEdit }) {
  return (
    <div style={{
      ...
      <div style={{
        ...
        {isCustom && (
          <button className="btn btn-secondary" onClick={() => onEdit(spell)} style={{ marginTop:4 }}>
            ✎ Edit Custom Spell
          </button>
        )}
      </div>
...
export default function SpellLibraryTab() {
  ...
  useEffect(() => {
    if (user?.id) loadCustomSpells(user.id);
  }, [user?.id]);
        ...
        {filtered.map(spell => (
          <button key={spell.isCustom ? `c-${spell.id}` : spell.name}
            ...
            <span style={{ flex:1, fontSize:'0.9rem', color:'var(--c-text)' }}>
              {spell.name}
              {spell.isCustom && <span className="badge badge-gold" style={{ marginLeft:6, fontSize:'0.5rem' }}>custom</span>}
            </span>
